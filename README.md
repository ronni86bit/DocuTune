...
- **Remote-run persistence (optional):** set `DOCUTUNE_HF_REPO_ID` (+ `DOCUTUNE_HF_TOKEN`, or `HF_TOKEN`) and the trainer mirrors **every checkpoint** to a **private** Hugging Face repo as an atomic, hash-verified commit (so a fresh Kaggle runtime resumes instead of restarting), then uploads the final adapter — with its training manifest and resolved config — after training. Disabled by default; local training needs no HF authentication; failed uploads are reported honestly and never touch local artifacts. See [docs/KAGGLE.md](docs/KAGGLE.md) for the full Kaggle workflow.
